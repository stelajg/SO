...
void helperAnalyzerIFDEF_ENDIF(int readKeys, int readUndef, FILE** fd_src, FILE* fd_dst, char** text, HashMap**map, char** keys,
                               int map_size, int forIfndef);
void analyzerLineInput(int* readKeys, int* readUndef, char* text, FILE* fd_src, FILE* fd_dst, HashMap** map,
                       char **keys, int* map_size, int flagIsFromOtherFunctions);
